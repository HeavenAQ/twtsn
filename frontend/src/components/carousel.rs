...
fn CarouselDot(
    ...
    view! { cx,
        <input
            type="radio" name="dot" id=format!("{}_{}", id_prefix, id)
...
fn CarouselLabel(
    ...
    view! { cx,
        <label
            ...
            style:border=move || format!("2px solid {}", if id == active_idx() { "white" } else { "gray" })
        />
    }
}
...
fn CarouselIdxWrapper(
    ...
    view! { cx,
        {carousel_range(slides_len).map(|id|
            view!{cx, <CarouselDot id_prefix="exhibit" id=id set_active_idx/> }
        ).collect_view(cx)}
        ...
        <div class="absolute bottom-5 flex left-1/2 -translate-x-1/2">
            {carousel_range(slides_len).map(|id|
                view!{cx, <CarouselLabel id_prefix="exhibit" id=id active_idx/> }
            ).collect_view(cx)}
        </div>
    }
}
        ...
        <div class="w-full h-full overflow-hidden relative shadow-lg rounded-xl">
            ...
            >
                <CarouselIdxWrapper
                    ...
                    set_active_idx
                >
                    {carousel_slides.iter().map(|slide| {
                        view! { cx,
                            <CarouselCard slide/>
                        }
                    }).collect_view(cx)}
